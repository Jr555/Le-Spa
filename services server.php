<?php 
	...
	if (isset($_POST['save'],$_GET['username'])) {
		...
		mysqli_query($db, "INSERT INTO service (username, description, price, duration, commission) VALUES ('$username','$description', '$price', '$duration', '$commission')"); 
		$_SESSION['message']; 
	...
	if (isset($_POST['update'])) {
		$id = $_POST['service_code'];
		$description = $_POST['description'];
		$price = $_POST['price'];
		$duration = $_POST['duration'];
		$commission = $_POST['commission'];
		$query = "SELECT username FROM service WHERE service_code = $service_code";
